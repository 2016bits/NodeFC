...
def make_personalization(entry_s, entry_n, entry_r=None, w_s=0.6, w_n=0.4, w_r=0.0):
    """Build personalization (teleport) distribution for PPR over heterogeneous nodes.
    ...
    Weights (w_s, w_n, w_r) are mixture weights across the three entry types.
    """
    p = defaultdict(float)

    for sid, sc in (entry_s or {}).items():
        p[f"S::{sid}"] += float(w_s) * max(float(sc), 0.0)

    for eid, sc in (entry_n or {}).items():
        p[f"N::{eid}"] += float(w_n) * max(float(sc), 0.0)

    if entry_r:
        for rid, sc in entry_r.items():
            p[f"R::{rid}"] += float(w_r) * max(float(sc), 0.0)
...
def predict_hop_heuristic(nlp, claim: str) -> int:
    """A lightweight hop predictor without training.
    Returns 2 for hop2 route, otherwise 3 (treated as hop3/4 route).
    """
    try:
        doc = nlp(claim)
        ents = [norm_ent(e.text) for e in doc.ents if e.text]
        ent_cnt = len(set([e for e in ents if e]))
    except Exception:
        ent_cnt = 0

    tok_len = len(claim.split())
    # Heuristic: multiple entities OR long complex claim -> multi-hop
    if ent_cnt >= 2 or tok_len >= 18:
        return 3
    return 2


def load_hop_model(model_name_or_path: str, device: str = "cuda"):
    """Load a hop classifier (claim -> hop label)."""
...
def _label_to_hop(label: str) -> int:
    """Robustly map model label string to hop number."""
    if label is None:
        return 3
    s = str(label).lower()
    # common patterns: 'hop2', '2', 'HOP_2'
    for h in (2, 3, 4):
        if f"hop{h}" in s:
            return h
        if re.fullmatch(rf"{h}", s):
            return h
        if re.fullmatch(rf"hop[_\- ]?{h}", s):
            return h
    return 3


@torch.no_grad()
def predict_hop_model(hop_tok, hop_model, claim: str, device: str = "cuda") -> int:
    inputs = hop_tok(claim, return_tensors="pt", truncation=True, max_length=256)
    inputs = {k: v.to(device) for k, v in inputs.items()}
    logits = hop_model(**inputs).logits
    pred = int(torch.argmax(logits, dim=-1).item())
    id2label = getattr(hop_model.config, "id2label", None) or {}
    label = id2label.get(pred, None)
    hop = _label_to_hop(label)
    # fallback: if labels are not informative, map 0/1/2 -> 2/3/4
    if hop == 3 and (label is None):
        num_labels = int(getattr(hop_model.config, "num_labels", 3))
        if num_labels == 3:
            hop = [2, 3, 4][pred]
        elif num_labels == 2:
            hop = 2 if pred == 0 else 3
    return hop


def relation_entry_r(claim: str, relation_nodes, topk: int = 15):
    """Simple relation entry scoring via keyword overlap on relation 'norm' field."""
    ck = extract_keywords_simple(claim)
    if not ck:
        return {}
    scored = []
    for r in relation_nodes:
        rid = r.get("rid", None)
        rn = r.get("norm", "") or r.get("name", "")
        if rid is None or not rn:
            continue
        rk = set(re.findall(r"[A-Za-z][A-Za-z\-']+", rn.lower()))
        ov = len(ck & rk)
        if ov > 0:
            scored.append((rid, float(ov)))
    scored.sort(key=lambda x: x[1], reverse=True)
    if not scored:
        return {}
    top = scored[:topk]
    # normalize to [0,1]
    mx = max(sc for _, sc in top) or 1.0
    return {rid: sc / mx for rid, sc in top}


def _collect_sid_meta(sent_nodes):
    sid2meta = {}
    for s in sent_nodes:
        sid = s.get("sid")
        txt = s.get("sentences") or s.get("text") or ""
        if sid is None or not txt:
            continue
        sid2meta[sid] = (txt, int(s.get("doc_rank", 10**9)), s.get("docid", None))
    return sid2meta


def hop2_local_retrieve(
    biencoder, crossencoder, nli_tokenizer, nli_model, nlp,
    claim, sent_nodes, sn_edges, entity_nodes, semantic_edges,
    entry_k_s=120, ce_topk=40, nli_topk=30, max_evidence=5,
    min_sen_sim=0.25,
    # diversify weights (hop2 defaults)
    alpha=1.2, beta=0.3, gamma=0.6, delta=0.7, eta=0.3,
    conn_sem_weight=0.1, conn_hard_theta=0.80, stop_tau=0.02,
    device="cuda"
):
    """Hop2 route: local precision without global PPR.
    Steps: bi-encoder Top-K -> cross-encoder -> NLI -> greedy diversification.
    """
    entry_s = semantic_entry_s(biencoder, claim, sent_nodes, topk=entry_k_s)

    sid2meta = _collect_sid_meta(sent_nodes)
    candidates = []
    for sid, sc in entry_s.items():
        if sid not in sid2meta:
            continue
        txt, doc_rank, _docid = sid2meta[sid]
        candidates.append((sid, float(sc), txt, doc_rank))

    candidates = sorted(candidates, key=lambda x: x[1], reverse=True)[:max(ce_topk, 1)]
    reranked = rerank_cross_encoder(crossencoder, claim, candidates, beta_graph=0.0)
    ...
    sim_map = build_semantic_sim_map(semantic_edges, min_sen_sim=min_sen_sim)
    ...
    diversified = diversify_greedy_select(
        ...
        delta=delta,
        eta=eta,
        conn_sem_weight=conn_sem_weight,
        conn_hard_theta=conn_hard_theta,
        stop_tau=stop_tau
    )
    return diversified, reranked, entry_s
...
def main(args):
    ...
    if args.hop_source == "model" and (not args.hop_model):
        raise ValueError("hop_source=model requires --hop_model pointing to a trained hop classifier")
    ...
    hop_tok, hop_model = None, None
    if args.hop_source == "model":
        hop_tok, hop_model = load_hop_model(args.hop_model, device=args.device)
    ...
    for ex_id in tqdm(keys):
        ...
        # hop-aware routing
        hop_hat = None
        if args.hop_source == "gold":
            hop_hat = int(data.get("num_hops", 3))
        elif args.hop_source == "model":
            hop_hat = int(predict_hop_model(hop_tok, hop_model, claim, device=args.device))
        else:
            hop_hat = int(predict_hop_heuristic(nlp, claim))

        # route: treat hop>=3 as multi-hop route
        hop_route = 2 if hop_hat <= 2 else 3

        if hop_route == 2:
            diversified, reranked, entry_s = hop2_local_retrieve(
                biencoder=biencoder,
                ...
                entry_k_s=args.h2_entry_k_s,
                ce_topk=args.h2_ce_topk,
                nli_topk=args.h2_nli_topk,
                max_evidence=args.max_evidence,
                min_sen_sim=args.min_sen_sim,
                alpha=args.h2_alpha_rel,
                beta=args.h2_beta_conn,
                gamma=args.h2_gamma_cov,
                delta=args.h2_delta_red,
                eta=args.h2_nli_eta,
                conn_sem_weight=args.h2_conn_sem_weight,
                conn_hard_theta=args.h2_conn_hard_theta,
                stop_tau=args.h2_stop_tau,
                device=args.device
            )
            ...
            entry_n = entity_entry_n(nlp, claim, entity_nodes)
            entry_r = relation_entry_r(claim, relation_nodes, topk=args.entry_k_r)

            p = make_personalization(
                entry_s, entry_n, entry_r,
                w_s=args.w_entry_s, w_n=args.w_entry_n, w_r=args.w_entry_r
            )
            ...
            # NLI rerank top-k: contradict high first
            reranked = rerank_by_nli(
                claim=claim,
                reranked=reranked,
                nli_tok=nli_tokenizer,
        ...
        results.append({
            ...
            "entry_rids": list(entry_r.keys()) if isinstance(entry_r, dict) else [],
            "hop_hat": hop_hat,
            "hop_route": hop_route,
    ...
    out_path = args.out_path.replace('[SPLIT]', args.split).replace('[HOP]', args.hop_source)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--in_path', type=str, default='./data/plan1/bm25_[SPLIT].json')
    ...
    parser.add_argument('--entry_k_r', type=int, default=15)
    parser.add_argument('--w_entry_r', type=float, default=0.0)  # multi-hop recommended ~0.30

    # hop-aware routing
    parser.add_argument('--hop_source', type=str, default='heuristic', choices=['heuristic', 'gold', 'model'])
    parser.add_argument('--hop_model', type=str, default='')  # required if hop_source=model

    # hop2 route (local precision)
    parser.add_argument('--h2_entry_k_s', type=int, default=120)
    parser.add_argument('--h2_ce_topk', type=int, default=40)
    parser.add_argument('--h2_nli_topk', type=int, default=30)

    parser.add_argument('--h2_alpha_rel', type=float, default=1.2)
    parser.add_argument('--h2_beta_conn', type=float, default=0.3)
    parser.add_argument('--h2_gamma_cov', type=float, default=0.6)
    parser.add_argument('--h2_delta_red', type=float, default=0.7)
    parser.add_argument('--h2_nli_eta', type=float, default=0.3)
    parser.add_argument('--h2_conn_sem_weight', type=float, default=0.1)
    parser.add_argument('--h2_conn_hard_theta', type=float, default=0.80)
    parser.add_argument('--h2_stop_tau', type=float, default=0.02)
